...
processNanostringData <- function(nsFiles,
  ...
  if (file.extension %in% c(".txt", ".TXT", ".csv", ".CSV")) {
    ...
    dat <- list(exprs = tabData[,-seq_len(12)],
                dict = tabData[,seq_len(12)])
    rownames(dat$exprs) <- rownames(dat$dict) <- tabData$TargetName
  ...
  dat$dict$CodeClass[dat$dict$TargetName %in% housekeeping] <- "Housekeeping"
